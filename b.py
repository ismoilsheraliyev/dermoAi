...
# ===================== CONFIG =====================
BOT_TOKEN = "7780653487:AAHfZD9Kq-pB4_i22pNQ-tokCIUPJk7do6c" 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_name = user_data.get('first_name') or user.first_name
...
⚙️ *Admin funksiyalari:*"""
        ...
        # Statistika
            ...
            stats_text = f"""{EMOJI['chart']} *{user_name.upper() if user_name else 'SHAXSIY'} STATISTIKANGIZ* {EMOJI['chart']}
                ...
                for reminder in reminders:
                    if (reminder.get('active', True) and 
                        ...
                        try:
                            # Foydalanuvchi ismini olish
                            user_data = get_user(user_id)
                            user_name = user_data.get('first_name', 'go\'zal')
                            
                            await context.bot.send_message(
                                chat_id=user_id,
                                text=f"{EMOJI['bell']} *{user_name}, kunlik eslatma!*\n\n{reminder['message']}",
                                parse_mode='Markdown'
                            )
